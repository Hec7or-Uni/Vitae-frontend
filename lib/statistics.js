...
import { __TODAY, toTimestamp, fromTimestamp } from './dates'
...
export function getUserNut (user) {
  ...
  for (const value of recipes) {
    ...
    _recipes.forEach(element => {
      ...
      if (dict.has(_date)) {
        dict.calories = (dict.calories + item.calories) || dict.calories
        dict.carbs = (dict.carbs + item.carbs) || dict.carbs
        dict.fat = (dict.fat + item.fat) || dict.fat
        dict.protein = (dict.protein + item.protein) || dict.protein
      } else {
        dict.set(_date, item)
      }
    })
...
export function getUserPreferences (user) {
  ...
  let carbs = 0; let fat = 0; let protein = 0
  meals.forEach(meal => {
